...
$journal_ids = explode(',', $_GET['journal_ids']);
...
$journalCount = $result->num_rows; // Track total number of journals
$currentJournal = 0; // Counter for current journal
...
while ($row = $result->fetch_assoc()) {
    ...
    $images = array_filter(
        array_map(fn($img) => !empty ($img) ? "../" . $img : null, [$row['journal_image1'], $row['journal_image2'], $row['journal_image3']]),
        fn($img) => !empty ($img)
    );
