...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $adviserId = isset($_POST['adviser_id']) ? (int) $_POST['adviser_id'] : 0;
    ...
    if (preg_match('/^[A-Z]{4}-[A-Z0-9]{2}$/', $courseName)) {
        ...
        if ($stmt = $database->prepare($checkQuery)) {
            ...
            if ($result->num_rows > 0) {
                ...
                // Check if adviser ID is valid
                if ($adviserId > 0) {
                    // Course section doesn't exist, proceed to insert
                    $insertQuery = "INSERT INTO course_sections (course_section_name, adviser_id) VALUES (?, ?)";
                    if ($stmt = $database->prepare($insertQuery)) {
                        $stmt->bind_param("si", $courseName, $adviserId);
                    ...
                    // Handle invalid adviser selection
                    $_SESSION['error_try'] = "Please select a valid adviser.";
                }
            }
